...
import yurisaki_bridge.transport as transport_module
...
def _record_event(message_id: int = 124) -> dict[str, object]:
    return _event(
...
@pytest.mark.asyncio
async def test_preview_waits_for_text_then_audio() -> None:
    client = FakeClient()
    transport = YurisakiTransport(client, _config(), wall_clock=lambda: 1_000.0)
    transport.start()
    request = asyncio.create_task(transport.request_preview("/a preview synthesis"))
    await _wait_for_calls(client, 1)

    text_event = _event(message_id=123)
    assert await transport.consume_event(text_event) is True
    assert request.done() is False
    assert await transport.consume_event(text_event) is False
    assert transport.was_consumed(text_event) is True
...
@pytest.mark.asyncio
async def test_preview_accepts_audio_then_text_or_one_combined_event() -> None:
    client = FakeClient()
    transport = YurisakiTransport(client, _config(), wall_clock=lambda: 1_000.0)
    transport.start()

    audio_first = asyncio.create_task(transport.request_preview("/a preview first"))
    await _wait_for_calls(client, 1)
    await client.emit(_record_event())
    assert audio_first.done() is False
    await client.emit(_event(message_id=125))
    assert len(await audio_first) == 2

    combined = asyncio.create_task(transport.request_preview("/a preview second"))
    await _wait_for_calls(client, 2)
    combined_message = [
        {"type": "text", "data": {"text": "曲目：Second"}},
...
@pytest.mark.asyncio
async def test_preview_wrong_sender_and_unrelated_segments_are_ignored() -> None:
    client = FakeClient()
    transport = YurisakiTransport(client, _config(), wall_clock=lambda: 1_000.0)
    transport.start()
    request = asyncio.create_task(transport.request_preview("/a preview test"))
    await _wait_for_calls(client, 1)

    assert await transport.consume_event(_event(user_id=999, message_id=120)) is False
    assert (
        await transport.consume_event(
            _event(
                message_id=121,
                message=[{"type": "image", "data": {"file": "cover"}}],
            )
        )
        is False
    )
    ...
    await client.emit(_event(message_id=122))
    await client.emit(_record_event(message_id=123))
    await request


@pytest.mark.asyncio
async def test_preview_collector_retains_only_bounded_required_fields() -> None:
    client = FakeClient()
    transport = YurisakiTransport(client, _config(), wall_clock=lambda: 1_000.0)
    transport.start()
    request = asyncio.create_task(transport.request_preview("/a preview test"))
    ...
    await client.emit(
        _event(
            message_id=140,
            message=[
                {"type": "unknown", "data": {"payload": large_value}},
                {"type": "text", "data": {"text": "曲目：Test", "raw": large_value}},
            ],
        )
    )
    await client.emit(
    ...
    assert large_value not in repr(events)


@pytest.mark.asyncio
async def test_preview_partial_response_timeout_is_incomplete_and_quarantined() -> None:
    client = FakeClient()
    transport = YurisakiTransport(
        client,
        _config(timeout_seconds=0.02, timeout_quarantine_seconds=5.0),
        wall_clock=lambda: 1_000.0,
    )
    transport.start()
    request = asyncio.create_task(transport.request_preview("/a preview test"))
...
@pytest.mark.asyncio
async def test_request_handles_python310_asyncio_timeout(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class LegacyAsyncioTimeoutError(Exception):
        pass

    async def legacy_wait_for(awaitable: object, **options: object) -> object:
        del awaitable, options
    ...
    monkeypatch.setattr(transport_module.asyncio, "wait_for", legacy_wait_for)
    transport = YurisakiTransport(FakeClient(), _config())
    transport.start()
...
@pytest.mark.asyncio
@pytest.mark.parametrize("first_command", ["/a info Test", "/a rand"])
async def test_preview_shares_single_flight_with_existing_commands(
    first_command: str,
) -> None:
    client = FakeClient()
    transport = YurisakiTransport(client, _config(), wall_clock=lambda: 1_000.0)
    transport.start()
    first = asyncio.create_task(transport.request(first_command))
    preview = asyncio.create_task(transport.request_preview("/a preview Test"))

    await _wait_for_calls(client, 1)
    assert len(client.calls) == 1
    await client.emit(_event(message_id=130))
    await first

    await _wait_for_calls(client, 2)
    assert client.calls[1][1]["message"] == "/a preview Test"
    await client.emit(_event(message_id=131))
    await client.emit(_record_event(message_id=132))
    await preview
