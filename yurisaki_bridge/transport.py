...
class YurisakiTransport:
    ...
    async def send_preview_record(
        ...
        *,
        ...
        """Send one captured NapCat record reference to the Tool caller."""
        ...
        try:
            await self._client.call_action(
                action,
                message=[{"type": "record", "data": {"file": file_reference}}],
                **destination,
            )
        except Exception as exc:
            raise SendFailedError("Failed to send preview record reference") from exc
